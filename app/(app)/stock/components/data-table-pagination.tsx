  ...
  return (
    <div className='flex items-center justify-between px-2'>
      <div className='flex-1 text-sm text-muted-foreground'>
        You have <span className='text-[#2A2A2A]'>{table.getFilteredRowModel().rows.length}</span> stock{' '}
          ...
          >
            <SelectTrigger className='h-8 w-auto p-0 border-none shadow-none focus:ring-0 focus:ring-offset-0 [&>svg]:hidden'>
              <span className='flex items-center ml-1 text-[#2A2A2A]'>
                ...
                <img src='/icons/downarrow.png' alt='chevron-down' className='h-2 w-2 mx-1' />
              </span>
            </SelectTrigger>
            <SelectContent side='top'>
              {[10, 20, 30, 40, 50].map((pageSize) => (
                <SelectItem key={pageSize} value={`${pageSize}`} className='text-[#2A2A2A]'>
                  {pageSize}
                </SelectItem>
              ))}
            </SelectContent>
